using BPUtil;
using ErrorTrackerServer.SQLiteMigration.Global.Model;
using SQLite;
using System;
...
namespace ErrorTrackerServer.SQLiteMigration.Global
{
	public class GlobalDb : DBBase
	{
		...
		public string DbFileFullPath { get { return Globals.WritableDirectoryBase + DbFilename; } }
		...
		{
			SQLiteConnection c = new SQLiteConnection(DbFileFullPath, true);
		...
		{
			if (startDate == 0 && endDate == 0)
				return Query<LoginRecord>("SELECT * FROM LoginRecord ORDER BY Date DESC");
			else
